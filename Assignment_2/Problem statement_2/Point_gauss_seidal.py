...
t_in=np.zeros((ny,nx),dtype=np.float)         # For plotting purpose (dummy)
t_k=np.zeros((ny,nx),dtype=np.float)          # Temperature matrix at kth iteration
t_inter=np.zeros((ny,nx),dtype=np.float)      # Intermediate Temperature matrix 
...
#-----------------------------------------------------------------------------------------#
#------------------------------Point Gauss Siedal Method----------------------------------#
#-----------------------------------------------------------------------------------------#
def point_gauss(t_k,b,nx,ny):
	c=2*(1+b)
	cc=0.01
	iterate=0
	err=[]
	ite=[]
	t_k1=np.zeros((ny,nx),dtype=np.float)
	...
	while(cc>=0.01):
		for j in range(1,ny-1):
			for i in range(1,nx-1):
				t_inter[j][i]=(1/c)*(t_k[j+1][i]+t_k1[j-1][i]+b*(t_k[j][i+1]+t_k1[j][i-1]))
		t_k1=np.add(t_in,t_inter)
	...
	plt.contourf(X,Y,np.transpose(t_k1),cmap='cividis')
	#plt.imshow(t_k1,cmap='cividis',interpolation='bilinear',origin='lower')
	plt.colorbar()
